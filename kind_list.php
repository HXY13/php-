...
<div class="head row">
    <div class="col-md-4 logo_box">
        <a href="book_main.php">
            <img src="img/logo.jpg" class="col-md-10 col-md-offset-1">
        </a>
    </div>
...
require ('test.php');
$aaa = $_GET['kind'];
$q = "SELECT * from more_book WHERE more_book_kind LIKE '%$aaa%'";
$r = @mysqli_query($con, $q);
...
for($i=0;$i<=$records;$i=$i+1)
{
    ...
    $a=$i*4;
    $b=$i*4+$display;
    $q = "SELECT * FROM more_book WHERE more_book_kind LIKE '%$aaa%' limit $a, $b";
    $r = @mysqli_query($con, $q);
    foreach ($r as $value2) {
